...
class Gui:
    ...
    def setup_figure_gui(self):
        ...
        self.figure_frm.grid_columnconfigure(tuple(range(n_cols + 1)), weight=1)

        ttk.Label(self.figure_frm, text="Wheel diameter in cm").grid(
        ...
        ttk.Label(self.figure_frm, text="Entire gear ratio").grid(
        ...
        ttk.Label(self.figure_frm, text="Moto name").grid(
            row=0, column=5, sticky="nesw"
        )

        self.moto_name_entry = ttk.Entry(self.figure_frm)
        self.moto_name_entry.grid(row=0, column=6, sticky="nesw")

        self.instant_power_label = tkinter.Label(
            self.figure_frm, fg="red", font=("Courier", 24)
        )
        self.instant_power_label.grid(row=1, column=0, sticky="nesw")
        self.instant_torque_label = tkinter.Label(
            self.figure_frm, fg="blue", font=("Courier", 24)
        )
        self.instant_torque_label.grid(row=1, column=1, sticky="nesw")

        self.communicates_label = ttk.Label(self.figure_frm)
        self.communicates_label.grid(row=1, column=2, sticky="nesw")

        ttk.Button(
            self.figure_frm, text="Open file", command=self.select_file_method
        ).grid(row=2, column=0, sticky="nesw")
        ttk.Button(
            self.figure_frm, text="Save figure", command=self.save_figure_method
    ...
    def setup_uart_gui(self):
        ...
        self.uart_btn = ttk.Button(
            self.uart_frame, text="Connect", command=self.connect_method
        )
        self.uart_btn.grid(row=0, column=0, sticky="we")
        self.disconnect_btn = ttk.Button(
            self.uart_frame,
            text="Disconnect",
            command=self.disconnect_method,
        ...
        ttk.Button(
            self.uart_frame, text="Refresh COMs", command=self.setup_com_dropdown
        ).grid(row=0, column=4)
        self.setup_com_dropdown()
    ...
    def setup_com_dropdown(self):
        ...
        self.com_dropdown_list = ttk.OptionMenu(
            self.uart_frame, self.value_inside, *port_list
        )
    ...
    def draw_figure(self):
        ...
        self.canvas.get_tk_widget().grid(
            row=3,
            columnspan=self.figure_column_span,
            rowspan=self.figure_row_span,
            sticky="news",
        )
