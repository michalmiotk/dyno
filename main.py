...
class Program:
    ...
    def is_valid_gear_ratio(self):
        try:
            ...
            log.debug(e)
            log.debug("not valid gear_ratio")
        else:
            return True
    ...
    def is_valid_wheel_diameter(self):
        try:
            ...
            log.debug(e)
            log.debug("not valid wheel_diameter")
        else:
            return True
    ...
    def select_file(self):
        ...
        add_to_df_with_wheel_torque(df, self.gui.get_wheel_diameter())
        add_to_df_power_in_KM(df)
        add_to_df_engine_rot_speed(
            df, self.gui.get_wheel_diameter(), self.gui.get_gear_ratio()
        )
    ...
    def print_chart_method(self):
        if self.gui.figure.is_figure_empty():
            ...
            return
        self.gui.clear_communicates_label()
        figure_filename = r"screenshots\print.png"
    ...
    def get_df_from_filtered_data_from_uart(
        ...
        add_to_df_with_wheel_torque(
            new_df, wheel_diameter_cm=self.gui.get_wheel_diameter()
        )
        add_to_df_power_in_KM(new_df)
        add_to_df_engine_rot_speed(
            new_df, self.gui.get_wheel_diameter(), self.gui.get_gear_ratio()
        )
        return new_df
        ...
        while self.update_gui_enabled:
            ...
            try:
                ...
                while not uart_process_q.empty():
                    ...
                    filter_data = raw_serial_row_to_filtered_data(raw_data)
                    if filter_data[0] < 0 or filter_data[1] < 0:
                        log.info(filter_data)
                    filter_data_list.append(filter_data)
                if len(filter_data_list) == 0:
                    continue
                df_from_recv_data = self.get_df_from_filtered_data_from_uart(
                    filter_data_list
                )
                power_in_KM, torque_in_Nm = self.filter_data_list_to_instant_labels(
                    df_from_recv_data
                )
                ...
                log.debug("adding to df done")
                self.uart_df = pd.concat([self.uart_df, df_from_recv_data])
                log.debug("concat done")
                self.gui.figure.update_figure(self.uart_df)
                log.debug("update figure done")
                self.gui.draw_figure()
                log.debug("draw figure done")
                time.sleep(0.5)
                log.info("update")
            except queue.Empty as e:
                log.debug(e)
            except UnicodeDecodeError as u:
                log.debug("unicode decode error" + str(u))
            except ValueError as v:
                log.debug("Value err" + str(v))

        log.debug("updating gui stopped")
    ...
    def start_get_data_thread(self):
        t1 = threading.Thread(
            target=self.get_data, args=(self.uart_process_queue, self.serial_object)
        )
        t1.daemon = True
        t1.start()
        ...
        while serial_obj.isOpen():
            ...
            if serial_obj.inWaiting() > 0:
                try:
                    ...
                    log.debug("serial data" + str(raw_data))
                    queue.put(raw_data)
                    ...
                    log.debug("UnicodeDecodeError" + str(unicodeDecode))
                    pass
